...
rm(list = ls())
# crash_value <- 999999
# nmodels <- 2 # how many models in the model averaging?
...
# numParallel <- 16 
# samp.size <- 8 #  # of samples for bootstrap and Monte Carlo simulation
# home.dir <- "c:/fda/mbbe/" # where to run the models,
# model.source <- "u:/fda/mbbe" # model control files will be in model.source\model1, model.source\model2 etc. 
...
# Args <- list(home.dir = "c:/fda/mbbe/",
#             model.source = "u:/fda/mbbe/",
#             nmodels = 2 ,
#             ngroups = 4 ,
#             samp.size = 8 ,
#             Reference.groups = c(1,2),
#             Test.groups = c(3,4),
#             numParallel = 16,
#             crash_value = 999999,
#             nmfe.path = "c:/nm744/util/nmfe74.bat",
...
#             rndseed = 1,
#             use.simulation.data = TRUE,
#             simulation.data.path  = "U:/fda/mbbe/data_sim.csv")
# Args.json <-  toJSON(Args)
# con <- file("u:/fda/mbbe/MBBEArgs.json")
# writeLines(Args.json, con)
# close(con)
library(RJSONIO)
library(dplyr)  
library(stringr) 
...
#' create boostrap samples of NONMEM data set, placed in home.dir, file names = data_sampN.csv
#' 
#' @param home.dir Folder where models are to be run
...
sample.data <- function(home.dir,nmodels,samp.size){
  ...
  for (this.samp in 1:samp.size){
    ...
    datafile.name <- paste0("data_samp",this.samp,".csv")
    ...
    for(this.model in 1:nmodels){
      ...
      newdata.line = paste0("$DATA ",file.path("..","..",paste0("data_samp",this.samp,".csv ", rest.of.data.line)))
...
run.bootstrap <- function(nmfe.path,home.dir,nmodels,samp.size,numParallel=availableCores()){
  pb = txtProgressBar(min = 0, max = samp.size, initial = 0) 
...
write.sim.controls <- function(home.dir,parms,base.models,samp.size,use.simulation.data,simulation.data.path=NULL){ 
  ...
  for(this.samp in 1:samp.size){
    ...
    if(dir.exists(sim.dir)){
      unlink(sim.dir,recurvise=TRUE,force=TRUE)
    }
    dir.create(sim.dir) 
...
wait.for.bs = function(nmodels, samp.size){
  total.models <- nmodels*samp.size
  models.done = 0
...
#' wait.for.sim
#' Pretty much the same as wait.for.bs, except no nmodels.
#' make array of all PIDs, check when all return Null 
...
#'
#' @examples
run.mbbe.json <- function(Args.json){ 
  Args <- fromJSON(Args.json)

  run.mbbe(Args$crash_value,Args$nmodels, Args$ngroups,Args$Reference.groups,
           Args$Test.groups,Args$numParallel, Args$samp.size, Args$home.dir,
           Args$model.source, Args$nmfe.path, Args$delta.parms, Args$use_check_identifiable,
           Args$NCA.end.time, Args$rndseed, Args$use.simulation.data, Args$simulation.data.path)
}
...
#' @param NCA.end.time 
#' @param rndseed 
#' @param use.simulation.data 
#' @param simulation.data.path 
#'
#' @return
#' @export
#'

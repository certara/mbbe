nmfe  <- "c:/nm74g64/util/nmfe74.bat"
samp_size <- 10
crash_value <- 10000
...
Get_SimNCA <- function(xml_file) {
  ...
  nmoutput <- processx::run(nmfe, args = c("sim1.mod", "sim1.lst"), wd = file.path(home.dir, "sim"))
...
tryCatch({
  ...
  if (!is.list(simparms)) {
    if (simparms == -1) {
      rval  <- c(crash_value, paste(xml_file, "not found"))
    }
    if (simparms == -2) {
      rval  <-c(crash_value, paste("Covariance results not available in", xml_file))
    }
    if (simparms == -3) {
      rval  <-c(crash_value, paste("Simulation failed for", home.dir))
    }
  } else {
    ...
    if(is.na(penalty)) penalty <- crash_value
    rval  <- c(penalty, "See above, written by R code") # text written by R, don't return any other text
  }
  },
  error = function(e){
    rval <-  c(crash_value, paste("Unknown Error in R in", home.dir,",",e))
})

rm(list = setdiff(ls(), "rval"))
a <- gc(verbose = FALSE)
rval

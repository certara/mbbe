...
$INPUT      ID TIME AMT {DURATION1[1]} CMT CMTT DV WT EGFR AGE COV1 COV2 EVID PERIOD GROUP TRT SEQ
...
CL 	= EXP(THETA(1)+THETA(3)*ETA(1)   {COVARVCL[1]} + BOVCL) ;; ETACLV IS CORRELATION OF ETA(V) AND ETA(CL)
V2 	= EXP(THETA(2)+THETA(4)*ETA(2)   {COVARVCL[2]} + BOVV) 
IF(TRT.EQ.1) THEN ;; REFERENCE
  KA 	= EXP(THETA(5) + BOVKA  {KAETA[1]})  
  F1	= 1
  {ALAG1[1]}
  {DURATION1[2]}
ELSE   ;; TEST
  KA 	= EXP(THETA(6) + BOVKA {KAETA[2]})  
  F1 	= EXP(THETA(7))
  {ALAG1[2]}
  {DURATION1[3]}
END IF   

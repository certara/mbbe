...
e.g. parallel design studies. If data from a well designed BE study were available,
Model based Virtual BE studies would not be needed.

#### Model uncetainty and model averaging

"All model are wrong, some models are useful" [5]. We interpret this
well known aphorism to mean that if a single model is used to describe the data, 
the prediction will be wrong (or at least not exactly correct). But, if "some" 
models (e.g., model averaging) are used, it may be useful, or, perhaps at least less
...
is used for model averaging. A set of bootstrap samples from the original data set are generated. NONMEM is then used to estimated parameters for each adequate model-bootstrap data

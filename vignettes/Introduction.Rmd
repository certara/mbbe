...
-   Clinical scenarios were rich sampling is not feasible, for example,
    pediatrics, critically ill patients.
...
models for Monte Carlo simulation is well established. Typically the
study design that gives rise to the data used for model development not
be a BE study, if it were then the simulated BE studies would not be
needed, as real data are available.
...
wrong. In general, we extend model uncertainty from a single model with
uncertain parameters to uncertainty about the model structure, also,
each with uncertain parameters. The workflow is:
...
1.  Develop model(s) of the data. Model development and criteria for an
    ...
    compartment rate constants etc). An adequate model typically would
    also include between occasion variability on Volume and elimination
    terms, if multiple periods are available in the observed data.

2.  If indicated, perform "model averaging" based on boot strap analysis
    of a set of selected adequate models.

3.  For each bootstrap sample, select the "best" model. In the current
    implementation, the best model is selected based on Bayesian
    information criteria (BIC). That is, each model is run using each
    bootstrap sample. For each sample, the bootstrap model, and
    associated parameter estimates, with the best BIC is selected. In
    this way, the uncertainty of both the model structure and the model
    parameters is described.

4.  From the selected models, perform Monte Carlo simulation, with a
    reasonable study design (e.g., 4 period, cross over, 50 subjects).

5.  Determine power of the analysis by calculating two one side t test
    statistics (6) on each simulated study.

There is general awareness that parameters for any given model are
estimated with uncertainty. In NONMEM as well as other software
...
with model averaging. This has been applied in covariate selection [7]
...
Aoki et. al [8] and illustrated below [![From Aoki, Y., Röshammar, D.,
Hamrén, B. et al. Model selection and averaging of nonlinear
...
formal BE study (e.g., 4 period, single dose cross over study) can be
simulated. The NCA parameters for each (simulated) subject in each study
are then calculated using and algorithm derived
...
subjects (demo graphics, sample time etc), but have different structure
models. The interpretation here is that there is a distribution of
models within the population (similar to the distributio of parameters),
so for any given subject, we have a prior value for the model, but
uncertainty about which model this subject follows.
...
thus there is some degree of "luck"associated with whehter or not the
formulae are deemed bio-equivalent. MBBE permits a more robust approach,
not dependent on a single draw from a distribution, that is, the power
of the given study design to find that the formulae are bioquivalent.
...
(11) in the replicateBE package. The resulting power then is simply the
...
-   The bootstrap analysis NONMEM control file will be the template for
    ...
    between the bootstrap analysis data set and the Monte Carlow
    simulation data set. Basically, the "best"model control file for
    each bootstrap sample is editted by replacine any \$TABLE records
    with those needed for the two one side T test, the \$EST replace
    with \$SIM and the final parameters copied from the .xml file into

...
-   SEQ
    ...
    Note, these data items must be provided in the analysis data set,
    even if they are not used. The GROUP (treatment and reference
...
file. In addition the \$DATA record is edited (grammatically by the R
...
structure of the analysis and simulation data sets must be the same.
...
model output files (.xml, .lst, .ext etc, as well as any any \$TABLE
output) will be
...
respectively, shere M is the model number and N is the bootstrap sample
